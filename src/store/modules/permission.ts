...
/**
 * 父路由的完整 path（$fullPath）是本身的 path
 * 子路由的完整 path（$fullPath）是父路由的 path 加子路由的 path
 * $fullPath 放在 meta 里，因为避免开发人员需要用到 fullPath，所以加个 $ 代表关键字
  ...
  return routes.map((route) => {
    if (!(route.meta && route.meta.$fullPath)) {
      ...
      if (route.path.startsWith("/")) {
        ...
        meta.$fullPath = fullPath!.replace(/\/+/g, "/");
      }
      route.meta = meta;
    }
    if (route.children && route.children.length > 0) {
      // 如果是 http 链接，不需要路由拼接
      if (isExternal(route.meta!.$fullPath)) {

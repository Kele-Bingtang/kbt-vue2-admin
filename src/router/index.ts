...
import { filterOnlyRolesRoutes } from "./utils";
...
/**
  ...
  meta: {
    ...
    hiddenInBread: boolean                    如果为 true，则该路由将不会出现在面包屑中，默认为 false
    hiddenInMenu: boolean                     如果为 true，则该路由不会显示在左侧菜单，默认为 false
...
const rolesRoutes: Array<RouteConfig> = [
  {
    path: "/",
    component: Layout,
    redirect: "/home",
  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        ...
        name: "test2",
        component: () => import("@/views/HomeView.vue"),
        meta: { title: "测试2", icon: "documentation" },
      },
      {
        path: "test3",
        name: "test3",
        component: () => import("@/views/HomeView.vue"),
        meta: { title: "测试3", icon: "documentation" },
      },
      {
        path: "test4",
        name: "test4",
        component: () => import("@/views/HomeView.vue"),
        meta: { title: "测试4", icon: "documentation" },
      },
    ],
  },
  {
...
router.beforeEach(async (to: Route, from: Route, next: any) => {
  let token = UserModule.token || getToken() || tokenMock;
  if (token) {
    if (to.path === "/login") {
      next({ path: "/" });
    } else {
      if (UserModule.roles.length === 0) {
        try {
          ...
          const onlyRolesRoutes = filterOnlyRolesRoutes(rolesRoutes, roles);
          PermissionModule.SET_ROUTES(onlyRolesRoutes);
